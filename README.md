...
Additional normal-hypothesis review after stage 49:

`NORMAL_HYPOTHESIS_COLLAPSE_TABLE_READY_NORMAL_EXPLANATIONS_WEAKENED_NOT_ATTACK_PROOF`

Japanese reading:

> 横展開クラスタ + A/B/Trial説明変数モデルは local artifact 上で支持。49番で通常解釈の多くは弱体化。ただし攻撃・意図・帰属・server-side 証明ではない。

## Scope

This package contains:

- sanitized CSV / JSON / TXT outputs from review stages **40b–49**
...
- a private-review plan for raw Manifest analysis without publishing raw Manifest.db files
...
| 49 | Normal-hypothesis collapse table |
...
This package can support a falsifiable forensic model of a mobile-native Apple platform-state anomaly cluster. It does **not** prove Trial abuse, Apple involvement, hidden MDM, remote command, attacker attribution, server-side trust-graph manipulation, backup poisoning, restore inheritance, Manifest modification, or intent.
...
## Raw Manifest material

Raw Manifest files are intentionally excluded from this public package. The private review direction is documented in `docs/RAW_MANIFEST_PRIVATE_REVIEW_PLAN.md`.
